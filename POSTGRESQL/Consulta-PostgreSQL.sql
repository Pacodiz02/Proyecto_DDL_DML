...
SELECT '29530596E','Angel Suarez','2003/12/16','Calle Fuengirola 1','M'
FROM EMPLEADOS
...
DELETE FROM PENALIZACIONES WHERE DNISocioP IN (SELECT DNI FROM SOCIOS WHERE EXTRACT(YEAR FROM age(now(),fecha_nacimiento))*12 + EXTRACT(MONTH FROM age(now(),fecha_nacimiento))<216);
...
-- (La función MAX(COUNT(*) no funciona en postgres)
...
-- (La función MINUS no funciona en Postgres)
